...
mutesoundeffects = False  # Change to true to allow sound effects to play.
...
class SkierClass(pygame.sprite.Sprite):
    ...
    def move(self, speed):
        ...
        self.rect.centerx = self.rect.centerx + (speedx * (baseSpeed / 10))
    ...
    for i in range(100):  # FIlls every space with a random block
        ...
        if not (location in locations):  # prevent 2 obstacles in the same place
            ...
            if (randy >= 0 and randy <= 9200):  # Stone 92%
                ...
                type = "furnace"
            elif randy > 9950 and randy <=9990 :  # Enchant .4%
                type = "enchant"
            else:
                type = "emerald" #Emerald .1%
            if type == "stone":
                img = "stone.png"
            elif type == "magma":
                img = "magma.png"
...
def animate():
    ...
    screen.blit(reservesCount, [64, 20])
...
while running and not dead:
    ...
    if hit:
        if hit[0].type == "stone" and not hit[0].passed:
            ...
            hit[0].passed = True
            diamond += diamondore * (float(enchant + 1) / 2)
            iron += ironore * (float(enchant + 1) / 2)
            reserves = 0
            diamondore = 0
            ironore = 0
            print furnace
        elif hit[0].type == "emerald" and not hit[0].passed:
            if health <= 10:     health += 10
            elif health < 20:    health += abs(health - 20)
            if not mutesoundeffects:
                pygame.mixer_music.load("pop.ogg")
                pygame.mixer_music.play()
            hit[0].passed = True
    ...
    enchantCount = font.render(str(float(enchant + 1) / 2), 1, (0, 0, 0))  # MOD
    reservesCount = font.render("Reserve Ores:" + str(reserves), 1, (0, 0, 0))  # MOD
    #print speed[1]
    animate()
